...
class HTTPClient(HTTPServer):
    ...
    def __init__(self, client_sock, client_addr):
        ...
        # Add seperate file and "ASCIISend" return values
        headers, f, asciiSend = self.create_response()
        #SEND file as is, isntead of enconding as utf-8
        if asciiSend == True:
            myFile = base64.b64decode(f)
            self.client_sock.send(headers.encode("utf-8")+myFile)
            print(headers, f)
        else:
            response = headers+f
            print(response)
    ...
    def create_response(self):
        ...
        if method == "GET" or method == "HEAD":
            try:
                ...
                #added content_type for ttf font
                elif ext == "ttf": content_type = "application/octet-stream"
                ...
                try:
                    ...
                    #send file without enconding as utf-8
                    asciiSend = True
                    baseFile = open("{}{}".format(HTTPServer.root_dir, uri), "rb").read()
                    ## need to base64encode file to be sent properly
                    f = base64.b64encode(baseFile)
                    length = len(baseFile)
                ...
                ### Moved length from resp format to each file read section
                resp = "HTTP/1.1 200 OK\r\nDate: {}\r\nServer: {}\r\nContent-Length: {}\r\n"\
                       "Content-type: {}\r\nConnection: Closed\r\n\r\n".format(datetime.now(), HTTPServer.server_name, length, content_type)

            except IOError:
                error = 404
        else:
            error = 405
        ## added file and ascii send return values
        return resp, f, asciiSend if not error else StatusCodeErrors.get_response(error, HTTPServer.server_name)
